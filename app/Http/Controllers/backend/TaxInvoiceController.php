...
class TaxInvoiceController extends Controller
{
    ...
    public function findItem(Request $request)
    {
        ...
        $stock= $project->stockCheck($project->id, $item->id);
    ...
    public function findItemIdsize(Request $request)
    {


        $item = ItemList::where('style_id', $request->style)->where('brand_id', $request->color)->where('group_no', $request->size)->first();
       return $item;
        $vat = 100 - (int)$item->vatRate->value;
        $unit_price = ($item->sell_price / 100) * $vat;
        $fifo = Fifo::where('item_id', $item->id)->where('remaining', '>', 0)->orderBy('id', 'ASC')->first();
    ...
    public function finalSaveInvoice(Request $request)
    {
        ...
        $journal->invoice_no=$invoice->invoice_no;
        ...
        $journal->narration= "Good sales on ". $invoice->pay_mode ;
        ...
        $journal_temps->account_head_id= $request->pay_mode=="Credit"? 19:219;
        $journal_temps->master_account_id= $request->pay_mode=="Credit"? 3:2;
        $journal_temps->account_head= AccountHead::where('id',  $request->pay_mode=="Credit"?19:219)->first()->fld_ac_head;
    ...
    {
        $invoice=Invoice::where('invoice_no',$request->invoice_no)->first();
        $invoice->date=$request->update_date;
        $invoice->save();
        $transection=StockTransection::where('transection_id',$invoice->id)->where('tns_type_code',"S")->update(['date'=>$invoice->date]);
        if ($request->ajax()) {
            return Response()->json([
                'invoice' => $invoice,
                'error' =>"Updated",
